function RWData(queryDiv, listDiv, editDiv, pageDiv) {
    ...
    this.setPage = function () {
        ...
        //});
        var rwData = this;
        this.pageDiv.find('a').each(function (i, item) {
            var url = $(item).attr('href');
            var n = url.indexOf('page=');
            var page = url.substring(n).replace('page=', '');
            $(item).attr('href', '#').click(function () {

                rwData.currentPage.val(page);
                rwData.queryForm.submit();
    ...
    this.bindData = function (pageIdx) {
        if (this.currentPage.length) {
            this.currentPage.val(pageIdx);
        }
